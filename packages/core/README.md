...
`--pa-icon` system — off Font Awesome `<i>` glyphs and text emoji — so icons match
their text colour, scale with font size, and re-skin from a single `--base-icon-*`
override across Pure Admin, the pure-css shell, and the web components.

- **Structural icons are now one masked `--pa-icon` system.** Every affordance (chevron, caret, search, refresh, filter, check, copy, ellipsis, close/clear/remove, edit/delete/add, save) draws an SVG mask in `currentColor` instead of a Font Awesome `<i>` or a text emoji.
- **New masked glyphs** — settings (cog), bell, user, and favorites (star). The settings-panel toggle, navbar notification bell, profile trigger, and profile-panel favorites tab moved off emoji / Font Awesome.
- **Unified severity icons.** Toasts, alerts, callouts, notifications, popconfirm and inline validation all draw one shared `.pa-icon--info/--success/--warning/--danger` family (circle-i / circle-check / triangle-alert / circle-x), consistent everywhere and re-skinnable in one place.
- **Per-surface severity overrides.** New `--pa-{toast,alert,callout,notifications}-icon-{sev}` hooks let a theme re-skin one surface's status glyph without moving the shared family.
